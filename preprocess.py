...
def load_databatch(data_folder, idx, img_size=8, is_train=True):
    if is_train:
    ...
    return (tf.reshape(tf.convert_to_tensor(x, dtype=tf.float64), shape=[data_size, img_size, img_size, 3]),
            tf.convert_to_tensor(Y_train, dtype=tf.string))

def get_data(train_file_path, test_file_path, batch_size=1000):
    train_set = train_file_path
    for i in range(10):
        train_data = tf.data.Dataset.from_generator(load_databatch, output_types=(tf.float64, tf.string),
                                                    args=(tf.Constant(train_file_path), tf.Constant(i+1)))
        if type(train_set) == str:
            train_set = train_data
        else:
            train_set.concatenate(train_data)
    validation_data = tf.data.Dataset.from_generator(load_databatch, output_types=(tf.float64, tf.string),
    ...
    validation_generator = test_datagen.flow(
        ...
        class_mode='categorical')
    """
    train_set.concatenate(train_set.map(tf.image.random_flip_left_right))
    train_set.concatenate(train_set.map(tf.image.random_flip_up_down))
    
    return train_set, validation_data

if __name__ == "__main__":
    train_gen, val_gen = get_data('Imagenet8_train', 'Imagenet8_val')
